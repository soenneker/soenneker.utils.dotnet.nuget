...
public sealed class DotnetNuGetUtil : IDotnetNuGetUtil
{
    ...
    public ValueTask<bool> Push(string packagePath, string? apiKey = null, string? source = "https://api.nuget.org/v3/index.json",
    ...
    {
        string argument = ArgumentUtil.Push(packagePath, apiKey, source, disableBuffering, noSymbols, noServiceEndpoint, skipDuplicate, timeout, symbolSource,
            symbolApiKey, verbosity);

        return _dotnetUtil.ExecuteCommand("nuget push", packagePath, path => argument, output =>
        {
    ...
    public ValueTask<bool> Delete(string packageName, string packageVersion, string? apiKey = null, string? source = "https://api.nuget.org/v3/index.json",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string argument = ArgumentUtil.Delete(packageName, packageVersion, apiKey, source, noServiceEndpoint, forceEnglishOutput, interactive, nonInteractive);

        // Execute the command and parse the output to determine success.
        return _dotnetUtil.ExecuteCommand("nuget delete", packageName, path => argument, output => { return true; }, null, log, cancellationToken);
    }
}